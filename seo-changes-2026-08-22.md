...
## 16. Affiliate product recommendations added to shin pads guide — 2 September 2026

Added a "Shin Pads Worth Trying" section to `content/football-gear/best-shin-pads-for-kids-football.mdx` (commit `c6257d7`), mirroring the existing affiliate pattern already live on `best-football-boots-for-wide-feet-kids.mdx`: an affiliate-disclosure line linking to `/editorial-policy`, three real named products, and `#affiliate-link-pending` placeholder links until real affiliate links are set up. Placed after "Types Of Shin Pads" (parallel to where the boots guide places "Boots Worth Trying" after "Best Boot Types"), and added to frontmatter `sections` so it appears in the sticky TOC.

Products picked to span the three types the article already covers rather than three variants of the same thing: **Nike Charge Shin Guards** (plain moulded slip-in, the default recommendation), **adidas Tiro Club Shin Guards** (budget moulded with elastic ankle straps, the middle ground for pads that slide), and **G-Form Pro-S Elite Shin Guards** (soft foam with extended ankle coverage, for younger/more physical age groups who find a hard shell restrictive). No pricing stated, matching the boots guide's own note that prices and stock aren't listed and change too often.

Slug/URL and all existing content unchanged - purely additive, no headings or internal links removed. `readTime` bumped 8 -> 9 (1,885 body words after the addition, ~9.4 min at 200wpm). `npm run build` passes, including static generation of `/football-gear/shin-pads/best-shin-pads-for-kids-football`.

No real affiliate links exist yet for any of the three products - same placeholder state as the boots guide. Follow-up needed once real affiliate links (Amazon Associates or equivalent) are set up: replace `#affiliate-link-pending` in both this article and the boots guide.
