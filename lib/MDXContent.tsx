...
const components = {
  ...
  p: ({ children }: any) => <p className="leading-8 mb-4">{children}</p>,
  ul: ({ children }: any) => <ul className="list-disc list-inside space-y-2 mb-6 ml-2">{children}</ul>,
  ol: ({ children }: any) => <ol className="list-decimal list-inside space-y-2 mb-6 ml-2">{children}</ol>,
  li: ({ children }: any) => <li className="leading-8">{children}</li>,
  strong: ({ children }: any) => <strong className="font-bold">{children}</strong>,
  em: ({ children }: any) => <em className="italic">{children}</em>,
  a: ({ children, href }: any) => (
    <a href={href} className="font-medium text-blue-700 underline underline-offset-4 hover:text-blue-900 transition">
      {children}
    </a>
  ),
  table: ({ children }: any) => (
  ...
  tr: ({ children }: any) => (
    <tr className="[&:nth-child(even)]:bg-gray-50">{children}</tr>
  ),
  th: ({ children }: any) => (
    <th className="border border-gray-300 p-3 text-left font-semibold bg-gray-100">{children}</th>
  ),
  td: ({ children }: any) => <td className="border border-gray-300 p-3">{children}</td>,
  blockquote: ({ children }: any) => (
    <blockquote className="border-l-4 border-blue-500 bg-blue-50 p-4 my-6">{children}</blockquote>
  ),
};
